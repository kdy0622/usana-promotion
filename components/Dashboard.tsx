...
import { 
  TrendingUp, Zap, ChevronRight, Star, Info, X, 
...
const Dashboard: React.FC<DashboardProps> = ({ userProfile }) => {
  ...
  useEffect(() => {
    const savedPoints = localStorage.getItem(`usana_points_${userProfile.name}`);
    if (savedPoints) {
  ...
  const savePoints = (id: string, value: number) => {
    ...
    setPoints(newPoints);
    localStorage.setItem(`usana_points_${userProfile.name}`, JSON.stringify(newPoints));
    setIsEditing(false);
  };
  ...
  return (
    <div className="p-4 space-y-6 animate-fadeIn pb-24">
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-indigo-100 text-sm">
              ...
              <span>평균 {userProfile.avgCP}CP</span>
            </div>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3 mt-6">
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-3 border border-white/10 text-center">
            <p className="text-[10px] uppercase tracking-wider text-indigo-200 font-bold">참여 프로모션</p>
            <p className="text-lg font-bold">8개 통합 관리</p>
          </div>
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-3 border border-white/10 text-center">
            <p className="text-[10px] uppercase tracking-wider text-indigo-200 font-bold">상태</p>
            <p className="text-lg font-bold text-yellow-300">A to Z 가이드</p>
          </div>
        </div>
      </section>
                ...
                <div className="flex justify-between items-start mb-2">
                  <div className="flex-1">
                    ...
                    <p className="text-[11px] text-slate-400 font-medium">
                      {currentVal} / {promo.targetValue} {promo.unit}
                    </p>
                  </div>
      ...
      <section className="bg-indigo-50 p-5 rounded-3xl border border-indigo-100 border-dashed">
        <div className="flex items-center gap-2 mb-2">
          <Zap className="w-5 h-5 text-indigo-600 fill-indigo-600" />
          <h4 className="font-bold text-indigo-800 tracking-tight">One Action, Multi-Reward</h4>
        </div>
        <p className="text-xs text-indigo-700 leading-relaxed font-medium">
          각 프로모션을 클릭하여 <span className="underline decoration-indigo-300 font-bold">상세 가이드(A-Z)</span>를 확인하세요. 
          점수를 입력하면 달성률이 실시간 반영됩니다.
        </p>
      </section>

      {selectedPromoIndex !== null && (
        <div className="fixed inset-0 z-[100] bg-slate-900/70 backdrop-blur-sm flex items-end justify-center animate-fadeIn">
          <div className="bg-white w-full max-w-md rounded-t-[40px] p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            <button 
              onClick={() => {
              ...
              <div className="space-y-4 animate-fadeIn">
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">
                    현재 달성 수치 입력 ({PROMOTIONS_DATA[selectedPromoIndex].unit})
                  </label>
                  <div className="flex gap-2">
                    ...
                    <button 
                      ...
                      className="bg-indigo-600 text-white px-6 rounded-2xl font-bold flex items-center gap-2 hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100"
                    >
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 p-3 rounded-2xl">
                    ...
                    <p className="text-xs font-bold text-slate-700">{PROMOTIONS_DATA[selectedPromoIndex].details.period}</p>
                  </div>
                  <div className="bg-slate-50 p-3 rounded-2xl">
                    <div className="flex items-center gap-2 text-slate-400 mb-1">
                    ...
                    <p className="text-xs font-bold text-slate-700">{PROMOTIONS_DATA[selectedPromoIndex].details.target}</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {PROMOTIONS_DATA[selectedPromoIndex].details.fullGuide.map((step, sidx) => (
                      <div key={sidx} className="flex gap-3 bg-slate-50/50 p-3 rounded-2xl border border-slate-100">
                ...
                <div className="pt-4 flex gap-3 pb-4">
                  <button 
                    ...
                    className="flex-1 bg-slate-900 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-slate-800 transition-all shadow-xl shadow-slate-200"
                  >
                    <Edit3 className="w-5 h-5" /> 실적 업데이트
                  </button>
                  <button 
